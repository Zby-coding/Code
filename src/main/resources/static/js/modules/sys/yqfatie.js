$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sysYqfatie/list',
        datatype: "json",
        colModel: [
			{ label: 'ID', name: 'id', index: "id", width: 45, key: true },
			{ label: '标题', name: 'name', index: "qcmc", width: 75 },
			{ label: '内容', name: 'content', index: "qcmc", width: 300 },
			{ label: '发帖人员', name: 'createuser', index: "qcmc", width: 75 },
			{ label: '发帖时间', name: 'createtime', index: "qcmc", width: 75 },
...
var vm = new Vue({
	...
	methods: {
		...
		add: function(){
			...
			vm.equipmentList = {};
			vm.title = "新增";
			vm.yqfatie = {deptName:null, deptId:null};
			vm.getDept();
		},
		update: function () {
		...
		getYqfatie: function(yqfatieId){
            $.get(baseURL + "sysYqfatie/info/"+yqfatieId, function(r){
            	vm.yqfatie = r.yqfatie;
				vm.getDept();
    		});
		},
		saveOrUpdate: function () {
			var url = vm.yqfatie.id == null ? "sysYqfatie/save" : "sysYqfatie/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.yqfatie),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(){
							vm.reload();
        ...
        getDept: function(){
            ...
            $.get(baseURL + "sys/dept/list", function(r){
                ...
                var node = dept_ztree.getNodeByParam("deptId", vm.yqfatie.deptid);
                if(node != null){
                    dept_ztree.selectNode(node);

                    vm.yqfatie.deptName = node.name;
                }
            })
        },
        deptTree: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级部门
                    vm.yqfatie.deptid = node[0].deptId;
                    vm.yqfatie.deptName = node[0].name;
	    ...
	    reload: function () {
			...
			$("#jqGrid").jqGrid('setGridParam',{ 
                postData:{'name': vm.q.yqfatieName},
                page:page
            }).trigger("reloadGrid");
		},
