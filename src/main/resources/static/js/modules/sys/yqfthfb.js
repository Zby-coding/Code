$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'sysYqfthfb/list',
        datatype: "json",
        colModel: [
			{ label: 'ID', name: 'id', index: "id", width: 45, key: true },
			{ label: '关联发帖父id', name: 'yqfatieid', index: "qcmc", width: 75 },
			{ label: '回复内容', name: 'content', index: "qcmc", width: 300 },
			{ label: '回复时间', name: 'createtime', index: "qcmc", width: 75 },
			{ label: '回复人员', name: 'createuser', index: "qcmc", width: 75 },
...
var vm = new Vue({
	...
	methods: {
		...
		add: function(){
			...
			vm.equipmentList = {};
			vm.title = "新增";
			vm.yqfthfb = {deptName:null, deptId:null};
			vm.getDept();
		},
		update: function () {
		...
		getYqfthfb: function(yqfthfbId){
            $.get(baseURL + "sysYqfthfb/info/"+yqfthfbId, function(r){
            	vm.yqfthfb = r.yqfthfb;
				vm.getDept();
    		});
		},
		saveOrUpdate: function () {
			var url = vm.yqfthfb.id == null ? "sysYqfthfb/save" : "sysYqfthfb/update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
                contentType: "application/json",
			    data: JSON.stringify(vm.yqfthfb),
			    success: function(r){
			    	if(r.code === 0){
						alert('操作成功', function(){
							vm.reload();
        ...
        getDept: function(){
            ...
            $.get(baseURL + "sys/dept/list", function(r){
                ...
                var node = dept_ztree.getNodeByParam("deptId", vm.yqfthfb.deptid);
                if(node != null){
                    dept_ztree.selectNode(node);

                    vm.yqfthfb.deptName = node.name;
                }
            })
        },
        deptTree: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择上级部门
                    vm.yqfthfb.deptid = node[0].deptId;
                    vm.yqfthfb.deptName = node[0].name;
	    ...
	    reload: function () {
			...
			$("#jqGrid").jqGrid('setGridParam',{ 
                postData:{'name': vm.q.yqfthfbName},
                page:page
            }).trigger("reloadGrid");
		},
